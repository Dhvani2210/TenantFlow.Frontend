    ...
    getTasksByProject(projectId, {
      ...
      .catch((err) => {
        if (err.response?.status === 404) {
          setError({ type: "notFound", message: "Project not found." });
        } else {
          setError({
            type: "generic",
                  ...
                  {error.type === "notFound" ? (
                    <div>
                      ...
                      <button
                        ...
                        className="px-4 py-2 text-sm font-medium rounded-lg bg-indigo-600 hover:bg-indigo-700 text-white transition-colors mt-4"
                      >
                        Go to Dashboard
                      </button>
                    </div>
